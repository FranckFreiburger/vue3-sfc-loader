...
:warning: **beware**, the following examples are sticky to version *<!--version-->0.9.4<!--/version-->*. For your use, you would prefer the [latest version](../README.md#dist)
...
<body>
  ...
  <script src="https://cdn.jsdelivr.net/npm/vue3-sfc-loader@0.9.4/dist/vue2-sfc-loader.js"></script>
  <script>
...
<body>
  <script type="module">
    ...
    import { loadModule } from 'https://cdn.jsdelivr.net/npm/vue3-sfc-loader@0.9.4/dist/vue3-sfc-loader.esm.js'
...
<body>
  ...
  <script src="https://cdn.jsdelivr.net/npm/vue3-sfc-loader@0.9.4/dist/vue3-sfc-loader.js"></script>
  <script>
...
[open in JSBin ▶](http://jsbin.com/?html,output&html=%3C!DOCTYPE+html%3E%0A%3Chtml%3E%0A%3Cbody%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Funpkg.com%2Fvue%403%2Fdist%2Fvue.runtime.global.prod.js%22%3E%3C%2Fscript%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Fcdn.jsdelivr.net%2Fnpm%2Fvue3-sfc-loader%400.9.4%2Fdist%2Fvue3-sfc-loader.js%22%3E%3C%2Fscript%3E%0A++%3Cscript%3E%0A%0A++++%2F*+%3C!--+*%2F%0A++++const+sfcContent+%3D+%60%0A++++++%3Ctemplate%3E%0A++++++++Hello+World+!%0A++++++%3C%2Ftemplate%3E%0A++++%60%3B%0A++++%2F*+--%3E+*%2F%0A%0A++++const+options+%3D+%7B%0A++++++moduleCache%3A+%7B%0A++++++++vue%3A+Vue%2C%0A++++++%7D%2C%0A++++++getFile(url)+%7B%0A%0A++++++++if+(+url+%3D%3D%3D+'%2FmyComponent.vue'+)%0A++++++++++return+Promise.resolve(sfcContent)%3B%0A++++++%7D%2C%0A++++++addStyle()+%7B+%2F*+unused+here+*%2F+%7D%2C%0A++++%7D%0A%0A++++const+%7B+loadModule+%7D+%3D+window%5B'vue3-sfc-loader'%5D%3B%0A++++Vue.createApp(Vue.defineAsyncComponent(()+%3D%3E+loadModule('%2FmyComponent.vue'%2C+options))).mount(document.body)%3B%0A%0A++%3C%2Fscript%3E%0A%3C%2Fbody%3E%0A%3C%2Fhtml%3E%0A)
<!--/example:target:cpn_string-->
[:top:](#readme)
...
[open in JSBin ▶](http://jsbin.com/?html,output&html=%3C!DOCTYPE+html%3E%0A%3Chtml%3E%0A%3Cbody%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Funpkg.com%2Fvue%403%2Fdist%2Fvue.runtime.global.prod.js%22%3E%3C%2Fscript%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Fcdn.jsdelivr.net%2Fnpm%2Fvue3-sfc-loader%400.9.4%2Fdist%2Fvue3-sfc-loader.js%22%3E%3C%2Fscript%3E%0A++%3Cscript%3E%0A%0A++++const+options+%3D+%7B%0A++++++moduleCache%3A+%7B+vue%3A+Vue+%7D%2C%0A++++++getFile%3A+()+%3D%3E+%60%3Ctemplate%3EHello+World+!%3C%2Ftemplate%3E%60%2C%0A++++++addStyle%3A+()+%3D%3E+%7B%7D%2C%0A++++%7D%0A++++Vue.createApp(Vue.defineAsyncComponent(()+%3D%3E+window%5B'vue3-sfc-loader'%5D.loadModule('file.vue'%2C+options))).mount(document.body)%3B%0A%0A++%3C%2Fscript%3E%0A%3C%2Fbody%3E%0A%3C%2Fhtml%3E%0A)
<!--/example:target:minimalist_example-->
[:top:](#readme)
...
[open in JSBin ▶](http://jsbin.com/?html,output&html=%3C!DOCTYPE+html%3E%0A%3Chtml%3E%0A%3Cbody%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Funpkg.com%2Fvue%403%2Fdist%2Fvue.runtime.global.prod.js%22%3E%3C%2Fscript%3E%0A++%3Cscript+src%3D%22https%3A%2F%2Fcdn.jsdelivr.net%2Fnpm%2Fvue3-sfc-loader%400.9.4%2Fdist%2Fvue3-sfc-loader.js%22%3E%3C%2Fscript%3E%0A++%3Cscript%3E%0A%0A++++%2F*+%3C!--+*%2F%0A++++const+sfcContent+%3D+%60%0A++++++%3Ctemplate%3E%0A++++++++Hello+World+!%0A++++++%3C%2Ftemplate%3E%0A++++%60%3B%0A++++%2F*+--%3E+*%2F%0A%0A++++const+options+%3D+%7B%0A++++++moduleCache%3A+%7B+vue%3A+Vue+%7D%2C%0A++++++async+loadModule(path)+%7B%0A%0A++++++++%2F%2F+(TBD)%0A%0A++++++%7D%2C%0A++++++getFile(url)+%7B%0A%0A++++++++if+(+url+%3D%3D%3D+'%2FmyComponent.vue'+)%0A++++++++++return+Promise.resolve(sfcContent)%3B%0A++++++%7D%2C%0A++++++addStyle()+%7B+%2F*+unused+here+*%2F+%7D%2C%0A++++%7D%0A%0A++++const+%7B+loadModule+%7D+%3D+window%5B'vue3-sfc-loader'%5D%3B%0A++++Vue.createApp(Vue.defineAsyncComponent(()+%3D%3E+loadModule('%2FmyComponent.vue'%2C+options))).mount(document.body)%3B%0A%0A++%3C%2Fscript%3E%0A%3C%2Fbody%3E%0A%3C%2Fhtml%3E%0A)
<!--/example:target:options_loadModule-->
[:top:](#readme)
...
```
<!--example:target:image_loading-->
[open in JSBin ▶](http://jsbin.com/?html,output&html=%3C!DOCTYPE+html%3E%0A%3Chtml%3E%0A%3Cbody%3E%0A%3Cscript+src%3D%22https%3A%2F%2Funpkg.com%2Fvue%403%2Fdist%2Fvue.runtime.global.prod.js%22%3E%3C%2Fscript%3E%0A%3Cscript+src%3D%22https%3A%2F%2Fcdn.jsdelivr.net%2Fnpm%2Fvue3-sfc-loader%400.9.4%2Fdist%2Fvue3-sfc-loader.js%22%3E%3C%2Fscript%3E%0A%3Cscript%3E%0A%0A++%2F*+%3C!--+*%2F%0A++const+config+%3D+%7B%0A++++files%3A+%7B%0A++++++%0A++++++%2F%2F+note%3A+Here%2C+for+convenience%2C+we+simply+retrieve+content+from+a+string.%0A%0A++++++'%2FtheComponent.vue'%3A+%60%0A++++++++%3Cscript+setup%3E%0A%0A++++++++++const+pngData+%3D+await+import('https%3A%2F%2Fupload.wikimedia.org%2Fwikipedia%2Fcommons%2F4%2F47%2FPNG_transparency_demonstration_1.png')%0A++++++++++%0A++++++++++%2F%2F+crate+an+ObjectURL+of+our+image%0A++++++++++const+pngBlobUrl+%3D+URL.createObjectURL(new+Blob(%5BpngData%5D))%3B%0A%0A++++++++++%2F%2F+cleanup+the+ObjectURL%2C+see+https%3A%2F%2Fdeveloper.mozilla.org%2Fen-US%2Fdocs%2FWeb%2FAPI%2FURL%2FcreateObjectURL_static%23memory_management%0A++++++++++const+onImageLoaded+%3D+(ev)+%3D%3E+URL.revokeObjectURL(ev.target.src)%3B%0A%0A++++++++%3C%2Fscript%3E%0A%0A++++++++%3Ctemplate%3E%0A++++++++++image+loaded+by+the+browser%2C+the+usual+way%3A%0A++++++++++%3Cimg+src%3D%22https%3A%2F%2Fupload.wikimedia.org%2Fwikipedia%2Fcommons%2F4%2F47%2FPNG_transparency_demonstration_1.png%22%2F%3E%0A++++++++++%3Chr%2F%3E%0A++++++++++image+loaded+by+vue3-sfc-loader+through+%3Cpre%3Efetch()%3C%2Fpre%3E%3A%0A++++++++++%3Cimg+%40load%3D%22onImageLoaded%22+%3Asrc%3D%22pngBlobUrl%22+%2F%3E%0A++++++++%3C%2Ftemplate%3E%0A%0A++++++++%3Cstyle%3E%0A++++++++++img+%7B+width%3A+128px%3B+vertical-align%3A+middle%3B+%7D%0A++++++++++pre+%7B+display%3A+inline%3B+%7D%0A++++++++%3C%2Fstyle%3E%0A++++++%60%2C%0A++++++%0A++++++'%2Fmain.vue'%3A+%60%0A++++++++%3Cscript+setup%3E%0A++++++++++import+theComponent+from+'%2FtheComponent.vue'%0A++++++++%3C%2Fscript%3E%0A++++++++%3Ctemplate%3E%0A++++++++++%3CSuspense%3E%0A++++++++++++%3CtheComponent%2F%3E%0A++++++++++%3C%2FSuspense%3E%0A++++++++%3C%2Ftemplate%3E%0A++++++%60%2C%0A++++%7D%0A++%7D%3B%0A++%2F*+--%3E+*%2F%0A%0A++const+options+%3D+%7B%0A++++devMode%3A+true%2C%0A++++moduleCache%3A+%7B%0A++++++vue%3A+Vue%2C%0A++++%7D%2C%0A++++async+getFile(url)+%7B%0A++++++%0A++++++if+(+config.files%5Burl%5D+)%0A++++++++return+config.files%5Burl%5D%3B%0A++++++%0A++++++const+res+%3D+await+fetch(url)%3B%0A++++++if+(+!res.ok+)%0A++++++++throw+Object.assign(new+Error(res.statusText+%2B+'+'+%2B+url)%2C+%7B+res+%7D)%3B%0A++++++return+%7B%0A++++++++getContentData%3A+asBinary+%3D%3E+asBinary+%3F+res.arrayBuffer()+%3A+res.text()%2C%0A++++++%7D%0A++++%7D%2C%0A%0A++++addStyle(textContent)+%7B%0A%0A++++++const+style+%3D+Object.assign(document.createElement('style')%2C+%7B+textContent+%7D)%3B%0A++++++const+ref+%3D+document.head.getElementsByTagName('style')%5B0%5D+%7C%7C+null%3B%0A++++++document.head.insertBefore(style%2C+ref)%3B%0A++++%7D%2C%0A%0A++++handleModule%3A+async+function+(type%2C+getContentData%2C+path%2C+options)+%7B%0A%0A++++++switch+(type)+%7B+%0A++++++++case+'.png'%3A%0A++++++++++return+getContentData(true)%3B+%2F%2F+load+as+binary%0A++++++%7D+%0A++++%7D%2C%0A%0A++++log(type%2C+...args)+%7B%0A%0A++++++console%5Btype%5D(...args)%3B%0A++++%7D%0A++%7D%0A%0A++const+app+%3D+Vue.createApp(Vue.defineAsyncComponent(()+%3D%3E+window%5B'vue3-sfc-loader'%5D.loadModule('%2Fmain.vue'%2C+options)))%0A++app.mount(document.body)%3B%0A%0A%3C%2Fscript%3E%0A%0A%3C%2Fbody%3E%0A%3C%2Fhtml%3E%0A)<!--/example:target:image_loading-->
